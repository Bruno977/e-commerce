...
describe('AddAttachmentToProductController (E2E)', () => {
  ...
  it('/POST /products/:productId/attachments', async () => {
    ...
    const result = await request(app.getHttpServer())
      .patch(`/products/${product.id}/attachments`)
    ...
    expect(updatedProduct?.attachments.length).toBe(2);
  });
});
